...
- **THEN** the system enters a pending continuation countdown for 10 seconds
...
- **WHEN** a 10-second pending continuation countdown is active
- **THEN** the system SHALL update the TUI toast once per second to reflect the remaining countdown
- **AND** the visible countdown sequence SHALL cover 10, 9, 8, 7, 6, 5, 4, 3, 2, and 1 seconds before injection occurs
...
- **THEN** the system SHALL start a fresh 10-second countdown for that later continuation
